...
class DisplayPanel extends JPanel {
    ...
    protected void paintComponent(Graphics g) {
        ...
        ys=20;
        g.drawString("Mergesort tracing tree:\n",xs,ys);
        ys=ys+10;;
        int start=0;
...
//traverses tree and computes x,y position of each node, stores it in the node
    ...
    public Node Split(Node root, ArrayList<String> s) {
        if (s.size()==1) {
            root = new Node(s.toString(), null, null);
            return root;
        }
        else {
            if(root == null)
                return Split(new Node(s.toString(), null, null),s);
            else {
                int mid = s.size()/2;
                ...
                for(int i =s.size()-1 ; i> mid-1 ; i--)
                    right.add(0,s.get(i));
                root.left = Split(root.left,left);
                root.right = Split(root.right,right);
                return root;
            }
        }
    }
    public void Merge() {
        ...
        ArrayList<String> mergedata = new ArrayList<String>();
        mergedata.add((String)node1.data);
        mergedata.add((String)node2.data);


        Node mergeNode = new Node(mergedata.toString(), null , null);
        node1.right= mergeNode;
        node2.left = mergeNode;
    }

//    public Node nextNode(Node given){
//
//    }
}
...
//    Node(String x, Node l, Node r, Node parent) {
//        left = l;
//        right = r;
//        data = (Object) x;
//        this.parent = parent;
